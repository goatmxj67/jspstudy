	...
	<script>
		...
		function fn_tableMaker(arr) {
			...
			for (let i = 0; i < arr.length; i++) {
				result += '<tr><td>' + arr[i].sno + '</td><td>' + arr[i].name + '</td><td>' + arr[i].age + '</td><td>' + arr[i].birthday + '</td><td>'+ arr[i].regdate + '</td><td></tr>';
			}
		...
		function fn_delete(){
			$('body').on('click', '#delete_btn', function(){  // 동적 객체이기때문에 'body'로 시작함 그래야 작동함
				// $(this) == '#delete_btn'
